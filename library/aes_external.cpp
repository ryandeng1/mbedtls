...
void AES::EncryptBlock(const unsigned char in[], unsigned char out[], const unsigned char key[])
{
...
void PerformAGMPCAES(unsigned char* key, unsigned char input_[16], unsigned char output_[16]) {
...
extern "C" {
  ...
  case 14:
    ...
    break;
  default:
    throw "Incorrect number of rounds";
  }
  rpc::client client(rpc_server_ip, 8080);
  auto res = client.async_call("PerformAGMPCAES");
  PerformAGMPCAES(key, input, output);
  ...
  if (key_bits == 128) {
      ...
      */
      unsigned char correct_output[16];
      AES aes(key_bits);
      aes.EncryptBlock(input, correct_output, key);      
      /*
      printf("Correct output\n");
      for (int i = 0; i < 16; i++) {
	  printf("%d ", correct_output[i]);
      }
      printf("\n");
      */
  ...
  if (key_bits != 128) {
      // For key-length other than 128 use the library AES
      AES aes(key_bits);
      aes.EncryptBlock(input, output, key);
  }
  return;
}
